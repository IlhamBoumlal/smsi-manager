...
import { 
  ShieldCheck, ArrowRight, Lightbulb, Play, Search, RotateCcw, 
  Zap, BarChart3, Users, LogIn, UserPlus, Mail, Phone, MapPin,
  ChevronRight
} from 'lucide-react';
      ...
      <header className="bg-white border-b border-blue-100 sticky top-0 z-50 shadow-sm">
        <div className="max-w-[1920px] mx-auto px-10 h-[80px] flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 flex-shrink-0">
            <button className="flex items-center gap-2 px-5 py-2 text-[16px] font-bold text-slate-700 hover:text-blue-600 transition-all">
              <LogIn size={18} />
              Connexion
            </button>
            <button className="px-8 py-3 text-[16px] font-bold rounded-full bg-slate-900 text-white shadow-lg hover:bg-blue-700 transition-all duration-200 active:scale-95">
              S'inscrire
            </button>
          </div>
        </div>
      ...
      <section style={{ background: "linear-gradient(135deg, #1e3a5f 0%, #2563eb 100%)", minHeight: "calc(100vh - 80px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem" }}>
        <div className="max-w-[1000px] mx-auto text-center">
          ...
          <div className="flex gap-5 justify-center">
            <button className="px-10 py-4 bg-white text-[#1e3a5f] rounded-2xl font-black text-lg shadow-xl hover:scale-105 transition-transform flex items-center gap-2">
               <UserPlus size={20}/> S'inscrire
            </button>
            <button className="px-10 py-4 border-2 border-white/30 text-white rounded-2xl font-black text-lg hover:bg-white/10 transition-colors">
               Se connecter
            </button>
          </div>
        </div>
