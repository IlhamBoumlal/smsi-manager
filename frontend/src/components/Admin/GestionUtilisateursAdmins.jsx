...
export default function GestionUtilisateursAdmins() {
  ...
  const [form, setForm] = useState({
    nomComplet: "", email: "", societeId: "", password: "", confirmPassword: "", isActive: true
  });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        setTimeout(() => { localStorage.removeItem('token'); localStorage.removeItem('user'); window.location.href = '/login'; }, 2000);
      } else {
  ...
  const handleSave = async () => {
    if (!form.nomComplet.trim()) { alert("Le nom complet est requis."); return; }
    if (!editing && !form.email.trim()) { alert("L'email est requis."); return; }
    if (!editing && !form.password) { alert("Le mot de passe est requis."); return; }
    if (!editing && form.password !== form.confirmPassword) { alert("Les mots de passe ne correspondent pas."); return; }
    if (!adminRoleId) { alert("Rôle 'Admin' non trouvé."); return; }
    ...
    try {
      ...
      setModalOpen(false);
      setEditing(null);
      setForm({ nomComplet: "", email: "", societeId: "", password: "", confirmPassword: "", isActive: true });
      setShowPwd(false);
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API_BASE}/api/user/${id}`, getAuthConfig());
      await fetchData();
    } catch (error) {
  ...
  try {
    ...
    const errorMsg = error.response?.data || "Erreur lors du changement de statut.";
  ...
  if (error) return (
    <div className="min-h-screen bg-[#f8f9fb] flex items-center justify-center p-6">
      <div className="bg-red-50 border border-red-200 rounded-2xl p-6 text-center max-w-md">
        ...
        <button onClick={fetchData} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm">Réessayer</button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#f8f9fb]" style={{ fontFamily: "'Sora', 'Segoe UI', sans-serif" }}>
      <div className="mx-auto max-w-[1400px] px-9 py-9 pb-16 w-full">
        ...
        <div className="flex justify-between items-center mb-7">
          ...
          <button onClick={() => { setEditing(null); setForm({ nomComplet: "", email: "", societeId: "", password: "", confirmPassword: "", isActive: true }); setModalOpen(true); }}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 mb-5 shadow-sm">
          ...
          <div className="flex flex-wrap justify-between items-center gap-3">
            ...
            <div className="flex border border-slate-300 rounded-xl overflow-hidden">
              <button onClick={() => setViewMode("grid")} className={`px-3 py-2 transition ${viewMode === "grid" ? "bg-blue-600 text-white" : "bg-white text-slate-600 hover:bg-slate-50"}`}><LayoutGrid size={17} /></button>
              <button onClick={() => setViewMode("table")} className={`px-3 py-2 transition ${viewMode === "table" ? "bg-blue-600 text-white" : "bg-white text-slate-600 hover:bg-slate-50"}`}><List size={17} /></button>
            </div>
          </div>
        </div>
            ...
            {filtered.map((user, idx) => (
              <div key={user.id} className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm hover:shadow-md transition" style={{ animation: `slideUp .5s cubic-bezier(.4,0,.2,1) ${idx * 60}ms both` }}>
                ...
                <div className="flex gap-2 mb-3">
                  <span className="px-2 py-1 bg-violet-50 text-violet-700 rounded-full text-xs border border-violet-200">{user.role || "Admin"}</span>
                ...
                <div className="flex gap-2">
                  <button onClick={() => { setEditing(user); setForm({ nomComplet: user.nomComplet, email: user.email, societeId: user.societeId || "", password: "", confirmPassword: "", isActive: user.isActive }); setModalOpen(true); }}
                  ...
                  <button onClick={() => handleDelete(user.id)} className="px-3 py-2 bg-red-50 text-red-500 rounded-lg hover:bg-red-100 transition"><Trash2 size={14} /></button>
                </div>
              </div>
            ))}
          </div>
                ...
                <tbody>
                  {filtered.map((user, i) => (
                    <tr key={user.id} className={`border-b border-slate-100 hover:bg-slate-50 transition ${i % 2 === 0 ? "bg-white" : "bg-slate-50/40"}`}>
                      ...
                      <td className="px-6 py-4"><span className="px-2 py-1 bg-violet-50 text-violet-700 rounded-full text-xs border border-violet-200">{user.role || "Admin"}</span></td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex justify-center gap-2">
                          <button onClick={() => { setEditing(user); setForm({ nomComplet: user.nomComplet, email: user.email, societeId: user.societeId || "", password: "", confirmPassword: "", isActive: user.isActive }); setModalOpen(true); }}
                            className="p-2 bg-blue-50 rounded-lg text-blue-600 hover:bg-blue-100 transition" title="Modifier"><Edit size={15} /></button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-[2px]">
            <div className="bg-white w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-xl">
              <div className="sticky top-0 px-6 py-4 border-b flex justify-between items-center rounded-t-2xl" style={{ background: GRAD_BLUE }}>
                ...
                <button onClick={() => setModalOpen(false)} className="p-2 hover:bg-white/15 rounded-lg transition"><X size={16} className="text-white" /></button>
              </div>
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="relative mt-1">
                      ...
                      <input value={form.nomComplet} onChange={e => setForm({...form, nomComplet: e.target.value})} type="text"
                    ...
                    <>
                      <div>
                        ...
                        <div className="relative mt-1">
                          ...
                          <input value={form.password} onChange={e => setForm({...form, password: e.target.value})} type={showPwd ? "text" : "password"}
                      ...
                      <div>
                        ...
                        <div className="relative mt-1">
                          <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={15} />
                          <input value={form.confirmPassword} onChange={e => setForm({...form, confirmPassword: e.target.value})} type={showPwd ? "text" : "password"}
                            className="w-full pl-9 pr-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                      </div>
                    </>
                    ...
                    <div className="col-span-2">
                      ...
                      <div className="flex gap-4 mt-1 p-3 border border-slate-300 rounded-lg bg-slate-50">
                        <label className="flex items-center gap-2">
                          <input type="radio" checked={form.isActive === true} onChange={() => setForm({...form, isActive: true})} className="w-4 h-4" /> Actif
                        </label>
                        <label className="flex items-center gap-2">
                          <input type="radio" checked={form.isActive === false} onChange={() => setForm({...form, isActive: false})} className="w-4 h-4" /> Inactif
                        </label>
                      </div>
                    </div>
                  )}
                ...
                <div className="flex gap-3 pt-4 border-t border-slate-200">
                  <button onClick={() => setModalOpen(false)} className="flex-1 py-2 border border-slate-300 rounded-lg text-sm hover:bg-slate-50 transition">Annuler</button>
                  <button onClick={handleSave} disabled={loading}
                    ...
                    {loading ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div> : <CheckCircle size={15} />}
