...
export default function GestionIncidents() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      // ✅ Utilise l'instance `api` avec le token JWT automatique
    ...
    <div style={{ minHeight: '100vh', background: T.bg, fontFamily: T.font }}>
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '36px 36px 60px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 28 }}>
          <div style={{ background: '#fff', borderRadius: 14, padding: '16px 20px', boxShadow: T.shadow }}>
            <div style={{ fontSize: 28, fontWeight: 800 }}>{stats.total}</div>
            <div style={{ fontSize: 12, color: T.gray500 }}>Total incidents</div>
          </div>
          <div style={{ background: '#fff', borderRadius: 14, padding: '16px 20px', boxShadow: T.shadow }}>
            <div style={{ fontSize: 28, fontWeight: 800, color: '#F59E0B' }}>{stats.enCours}</div>
            <div style={{ fontSize: 12, color: T.gray500 }}>En cours</div>
          </div>
          <div style={{ background: '#fff', borderRadius: 14, padding: '16px 20px', boxShadow: T.shadow }}>
            <div style={{ fontSize: 28, fontWeight: 800, color: '#10B981' }}>{stats.resolus}</div>
            <div style={{ fontSize: 12, color: T.gray500 }}>Résolus</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
          <div style={{ position: 'relative', flex: 1 }}>
            ...
            <input type="text" placeholder="Rechercher un incident..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
              style={{ width: '100%', padding: '10px 10px 10px 40px', borderRadius: 40, border: `1.5px solid ${T.gray200}`, fontSize: 14 }} />
          </div>
          <select value={filterStatut} onChange={e => setFilterStatut(e.target.value)}
            style={{ padding: '10px 16px', borderRadius: 40, border: `1.5px solid ${T.gray200}`, background: '#fff', fontSize: 13 }}>
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          ...
          {!loading && filtered.length === 0 && (
            <div style={{ textAlign: 'center', padding: '80px 0' }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 700 }}>Aucun incident trouvé</div>
            </div>
          )}
          {!loading && filtered.map((incident, idx) => (
            <div key={incident.id}
              ...
              <div style={{ padding: '20px 22px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 12, color: T.gray400 }}>
