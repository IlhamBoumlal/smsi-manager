...
// Menu Admin : accessible à tout utilisateur ayant le rôle "Admin"
const allAdminMenuItems = [
  { label: "Gestion des utilisateurs et permissions", Icon: Users, path: "/admin-societe" },
];
...
export default function Header({ activeAxe: activeAxeProp, onAxeChange }) {
  ...
  if (user) {
    const logoPath = user.logoUrl || user.logo || user.societeLogo || user.societe?.logoUrl || user.societe?.logo;
        ...
        <div className="max-w-[1920px] mx-auto px-6 h-[85px] flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <div className="flex flex-col leading-tight">
              <span className="text-xl font-black text-[#1e3a5f] tracking-tight">SMSI <span className="text-blue-600">Manager</span></span>
    ...
    return (
      <header className="bg-white border-b border-blue-100 sticky top-0 z-50 shadow-md font-sans">
        <div className="max-w-[1920px] mx-auto px-6 h-[85px] flex items-center justify-between gap-4">
          ...
          <button onClick={() => { logoutUser(); navigate("/"); }} className="flex items-center gap-2 px-4 py-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors">
  ...
  return (
    <header className="bg-white border-b border-blue-100 sticky top-0 z-50 shadow-md font-sans">
      <div className="max-w-[1920px] mx-auto px-6 h-[85px] flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 flex-shrink-0 cursor-pointer" onClick={() => handleAxeChange({ id: "tableau-bord", path: "/tableau-bord", moduleCode: "dashboard" })}>
          <img src={logoImage} alt="Logo" className="h-12 w-auto object-contain" />
          <div className="flex flex-col leading-tight">
            <span className="text-xl font-black text-[#1e3a5f] tracking-tight">SMSI <span className="text-blue-600">Manager</span></span>
            <span className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">ISO 27001</span>
        ...
        {mainAxes.length > 0 && (
          <nav className="flex items-center gap-2 flex-1 justify-center">
            {mainAxes.map((axe) => <NavButton key={axe.id} label={axe.label} isActive={activeAxe === axe.id} onClick={() => handleAxeChange(axe)} />)}
            {moreAxes.length > 0 && (
              <div ref={dropdownRef} className="relative">
                <NavButton label="Plus" isActive={isMoreActive || dropdownOpen} onClick={() => setDropdownOpen(p => !p)} suffix={<ChevronDown size={18} className={`transition-transform duration-300 ${dropdownOpen ? "rotate-180" : ""}`} />} />
                {dropdownOpen && (
                  <div className="absolute left-0 top-full mt-4 w-60 bg-white rounded-2xl border border-blue-100 shadow-2xl overflow-hidden z-50 py-2">
                    {moreAxes.map((axe) => <DropdownItem key={axe.id} axe={axe} isActive={activeAxe === axe.id} onClick={() => { handleAxeChange(axe); setDropdownOpen(false); }} />)}
                  </div>
                )}
              </div>
            )}
          ...
          {user ? (
            <div ref={userMenuRef} className="relative">
              <button onClick={() => setUserMenuOpen(p => !p)} className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-slate-50 transition-colors">
                <div className="w-11 h-11 rounded-full bg-gradient-to-br from-blue-600 to-blue-800 text-white flex items-center justify-center text-sm font-bold shadow-lg shrink-0">{initiales}</div>
                ...
                <div className="absolute right-0 top-full mt-4 w-72 bg-white rounded-2xl border border-slate-100 shadow-2xl overflow-hidden z-50">
                  <div className="px-5 py-5 bg-[#1e3a5f]">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-full bg-white/20 text-white flex items-center justify-center text-base font-bold shrink-0">{initiales}</div>
                  ...
                  {/* Menu Admin : affiché si l'utilisateur a le rôle "Admin" */}
                  {isAdmin && (
                    <div className="py-2 border-b border-slate-100">
                      {allAdminMenuItems.map(({ label, Icon, path }) => (
                        <button key={path} onClick={() => { navigate(path); setUserMenuOpen(false); }} className="w-full flex items-center gap-4 px-5 py-3 text-[15px] font-medium text-slate-700 hover:bg-slate-50 transition-colors">
                          <span className="w-9 h-9 rounded-lg bg-blue-50 flex items-center justify-center shrink-0"><Icon size={18} className="text-blue-600" /></span>
                          {label}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="py-2">
                    <button onClick={() => { logoutUser(); navigate("/"); }} className="w-full flex items-center gap-4 px-5 py-3 text-[15px] font-bold text-red-500 hover:bg-red-50 transition-colors">
            ...
            <button onClick={() => navigate("/login")} className="flex items-center gap-2 px-8 py-3 text-[15px] font-bold rounded-full bg-slate-900 text-white shadow-xl hover:bg-blue-700 transition-all active:scale-95">
...
function NavButton({ label, isActive, onClick, suffix }) {
  return (
    <button onClick={onClick} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-[15px] font-bold transition-all duration-200 whitespace-nowrap ${isActive ? "bg-blue-600 text-white shadow-lg shadow-blue-200" : "text-slate-600 hover:bg-blue-50 hover:text-blue-600"}`}>
      {label} {suffix}
    </button>
  );
}
...
function DropdownItem({ axe, isActive, onClick }) {
  return (
    <button onClick={onClick} className={`w-full flex items-center gap-4 px-6 py-4 text-[15px] font-bold text-left transition-all ${isActive ? "bg-blue-50 text-blue-600" : "text-slate-600 hover:bg-blue-50 hover:text-blue-600"}`}>
